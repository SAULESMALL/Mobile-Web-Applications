document.addEventListener('DOMContentLoaded', () => {
    ...
    // Log retrieved selections
    console.log("Retrieved Selections: ", { selectedMood, selectedEnergy, selectedDiet });
    ...
    if (filteredRecipes.length > 0) {
        recipeList.innerHTML = filteredRecipes.map(recipe => `
            <div class="recipe-card">
                ...
                <p>Energy Level: ${recipe.energyLevel}</p>
                <p>Diet: ${recipe.dietaryPreference}</p>
                <button onclick="window.location.href='recipes/${recipe.name.toLowerCase().replace(/ /g, '-')}.html'">Cook Me</button>
            </div>
        `).join('');
    } else {
        recipeList.innerHTML = `
            <p>No recipes match your selections. Please try different options.</p>
